...
$root_path = storage_path('app/files/' . Auth::id());
...
// get path
$p = isset($_GET['p']) ? $_GET['p'] : (isset($_GET['p']) ? $_GET['p'] : '');
...
if (isset($_GET['file'], $_GET['copy_to'], $_GET['finish'])) {
    // from
    $path = FM_ROOT_PATH;
    if (FM_PATH != '') {
        $path .= '/' . FM_PATH;
    ...
    $copy_to = fm_clean_path($_GET['copy_to']);
    if ($copy_to != '') {
...
// Mass deleting
if (isset($_GET['group'], $_GET['delete'])) {
    $path = FM_ROOT_PATH;
    if (FM_PATH != '') {
        $path .= '/' . FM_PATH;
    }
...
if (isset($_GET['upload'])) {
    ...
    <div class="path">
        ...
        <form action="" method="get" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button class="btn"><i class="fa fa-check"></i> Upload</button> &nbsp;
    ...
    <div class="path">
        ...
        <ul class="folders break-word">
            ...
            foreach ($folders as $f) {
                ?>
                <li><a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-folder fa-2x"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a></li>
            <?php
            }
            ?>
        </ul>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <table class="table">
            <tr><th>Full path</th><td> <?php echo fm_enc(fm_convert_win($file_path)) ?></td></tr>
            <tr><th>File size</th><td> <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?></td></tr>
            <tr><th>MIME-type</th><td> <?php echo $mime_type ?></td></tr>
            <?php
            // ZIP info
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                <tr><th>Files in archive</th><td> <?php echo $total_files ?></td></tr>
                <tr><th>Total size</th><td> <?php echo fm_get_filesize($total_uncomp) ?></td></tr>
                <tr><th>Size in archive</th><td> <?php echo fm_get_filesize($total_comp) ?></td></tr>
                <tr><th>Compression</th><td> <?php echo round(($total_comp / $total_uncomp) * 100) ?>%</td></tr>
                <?php
            }
            // Image info
            if ($is_image) {
                $image_size = getimagesize($file_path);
                echo '<tr><th>Image sizes</th><td>' . (isset($image_size[0]) ? $image_size[0] : '0') . ' x ' . (isset($image_size[1]) ? $image_size[1] : '0') . '</td></tr>';
            }
            // Text info
            if ($is_text) {
                ...
                echo '<tr><th>Charset</th><td>' . ($is_utf8 ? 'utf-8' : '8 bit') . '</td></tr>';
            }
            ?>
        </table>
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="fa fa-download"></i> Download</a></b> &nbsp;
            <b><a href="{{ route('edit', ['path' => (FM_PATH == '') ? 'root' : urlencode('root/'.FM_PATH), 'file' => urlencode($file)]) }}" target="_blank"><i class="fa fa-link"></i> Open</a></b> &nbsp;
...
<th style="width:3%"><input type="checkbox" title="Invert selection" onclick="checkbox_toggle()"></th>
...
<th style="width:13%"></th></thead>
<?php
// link to parent folder
if ($parent !== false) {
    ?>
<tbody><tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode($parent) ?>"><i class="icon-arrow_up"></i> ..</a></td></tr>
<?php
}
foreach ($folders as $f) {
    $is_link = is_link($path . '/' . $f);
    $img = $is_link ? 'icon-link_folder' : 'fa fa-folder fa-2x text-warning';
...
<p><input class="btn btn-dark bg-danger" type="submit" name="delete" value="Delete" onclick="return confirm('Delete selected files and folders?')">
<input class="btn btn-dark bg-primary" type="submit" name="zip" value="Pack" onclick="return confirm('Create archive?')">
<input class="btn btn-dark bg-primary" type="submit" name="copy" value="Copy"></p>
</form>
...
function fm_get_file_icon_class($path)
{
    ...
    switch ($ext) {
        ...
        case 'map': case 'lock': case 'dtd':
            $img = 'fa fa-file fa-2x';
            break;
        case 'zip': case 'rar': case 'gz': case 'tar': case '7z':
            $img = 'fa fa-file-archive fa-2x';
            break;
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb align-items-center pl-5 h5">
        <?php
        $path = fm_clean_path($path);
        $root_url = "<li class='breadcrumb-item'><a href='?p='><i class='fa fa-home' title='" . FM_ROOT_PATH . "'></i> Home</a></li>";
        if ($path != '') {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $array = "<li class='breadcrumb-item'><a href='?p={$parent_enc}'>" . fm_enc(fm_convert_win($exploded[$i])) . "</a></li>";
            }
            $root_url .= $array;
        }
        echo $root_url;
    ...
    <a class="btn btn-primary" title="Upload files" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="fa fa-upload"></i> Upload</a>
    <a class="btn btn-primary" title="New folder" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="fa fa-folder"></i> New Folder</a>
...
function fm_show_header()
{
    ...
    <!-- Favicons -->
    <link rel="icon" href="{{ asset('img/favicons/favicon.svg') }}">
